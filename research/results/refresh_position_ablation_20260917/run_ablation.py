"""Closed-loop marginal refresh-position ablation on development prompts."""
import argparse
...
def main():
    ...
    manifest = dict(
        experiment="Closed-loop marginal refresh-position ablation",
        ...
        uniform9_refresh_indices=UNIFORM9_REFRESH,
        dropped_refresh=DROPPED_REFRESH, refresh_indices=SCHEDULES,
        ablation_design=("Each eight-call mode removes exactly one refresh from "
                         "uniform9 and creates a five-step span between unchanged anchors."),
        ...
        source_hashes={p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in
                       [Path(__file__), source_dir / "position_schedules.py",
    ...
    if args.resume:
        ...
        for key in ("experiment", "cases", "modes", "prompts", "held_out_used",
                    "steps", "cache_branch_id", "dropped_refresh", "refresh_indices"):
    ...
    for case_index, (case, prompt_id, prompt, seed) in enumerate(cases):
        ...
        for mode in order:
            ...
            row = dict(
                ...
                dropped_refresh=DROPPED_REFRESH[mode],
    ...
    display_modes = ("dpm20", "uniform9") + MODES
    for prompt_id in sorted({case[1] for case in cases}):
        ...
        for y, (case, _, _, _) in enumerate(prompt_cases):
            for x, mode in enumerate(display_modes):
                path = ((REFERENCE_DIR / f"{case}_{mode}.png") if mode in ("dpm20", "uniform9")
                        else (args.output / f"{case}_{mode}.png"))
                canvas.paste(Image.open(path).convert("RGB"), (x*512, y*550+38))
                calls = reference[case, mode]["full_calls"] if mode in ("dpm20", "uniform9") else 8
