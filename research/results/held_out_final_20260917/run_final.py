"""One-time held-out validation of the frozen DeepCache candidates."""
import argparse
...
def build_cases(prompts, split="held_out_not_run"):
    return [(f'{prompt["id"]}_{seed}', prompt["id"], prompt["prompt"], seed)
            for prompt in prompts[split] for seed in prompts["seeds"]]
...
def main():
    ...
    parser.add_argument("--limit", type=int, default=0,
                        help="Case limit allowed only with --smoke-analysis")
    parser.add_argument("--smoke-analysis", action="store_true",
                        help="Use development prompts for pre-run validation")
    ...
    if args.limit and not args.smoke_analysis:
        raise ValueError("--limit is allowed only with --smoke-analysis")
    split = "analysis" if args.smoke_analysis else "held_out_not_run"
    cases = build_cases(prompts, split=split)
    if args.limit:
    ...
    manifest = dict(
        experiment=("Development smoke test for held-out runner" if args.smoke_analysis
                    else "One-time held-out final validation"),
        model_path=model_path, cases=cases, modes=MODES, prompts=prompts,
        held_out_used=not args.smoke_analysis, height=512, width=512, guidance_scale=7.5,
        ...
        source_hashes={p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in
                       [Path(__file__), source_dir.parent / "stage_aware_v2_dev_20260915" /
                        "stage_aware_cache.py", prompts_path, source_dir / "PROTOCOL.md"]},
        ...
        frozen_decision_rule={
            "primary_contrast": "drop_middle8 minus uniform9 paired SSIM",
            "acceptable_mean_ssim_loss": 0.01,
            "acceptable_prompt_loss_threshold": 0.03,
            "maximum_prompts_beyond_threshold": 1,
            "compute_requirement": "drop_middle8 must use 8 full calls and be faster on average",
        },
        note=("One-time held-out evaluation. Uniform9 and stage_aware9 use nine full "
              "UNet calls, drop_middle8 uses eight, and fixed3 uses seven. Results "
              "must not be used to retune a replacement policy on these prompts."),
    )
